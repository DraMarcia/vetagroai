...
const AnaliseImagens = () => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast({
      ...
      description: `${loadedImages.length} imagem(ns) pronta(s) para análise.`,
    });
  };
  ...
  const handleAnalyze = async () => {
    ...
    if (isProfessional === "sim" && !crmv.trim()) {
      toast({
        title: "Campo obrigatório",
        description: "Por favor, informe seu número de CRMV.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (images.length === 0 || !symptoms.trim()) {
      toast({
        ...
        description: "Envie pelo menos uma imagem e descreva os sintomas.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const { data, error } = await supabase.functions.invoke("analyze-equine", {
        body: {
          images: images,
          breed: isProfessional === "sim" ? `Profissional - CRMV: ${crmv}` : "Análise para tutor/produtor",
          age: symptoms,
          purpose: "Análise de imagens médicas veterinárias (ocular, sinais clínicos, exames)",
        },
      });
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Análise de Imagens Veterinárias</h1>
            <p className="text-muted-foreground">Análise de imagens oculares, sinais clínicos e exames</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 max-w-3xl">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Imagens para Análise</CardTitle>
            <CardDescription>
              Envie imagens oculares, de sinais clínicos ou exames (raio-x, ultrassom, PDF)
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Descreva detalhadamente os sintomas e sinais clínicos observados no animal
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div>
                ...
                <Textarea
                  id="symptoms"
                  placeholder="Ex: Animal apresentando secreção ocular, letargia, diminuição do apetite..."
                  value={symptoms}
        ...
        {result && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Até 4 diagnósticos prováveis baseados nas imagens e sintomas
              </CardDescription>
            </CardHeader>
            <CardContent>
